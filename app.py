...
LOGROS_DATA = {
    ...
    "hattrick": {"icon": "🎯", "name": "Hat-Trick", "desc": "3+ resultados exactos."},
...
if not st.session_state.autenticado:
    ...
    with tabs[2]: # RANKING CON FRASES ROTATORIAS
        tipo_rank = st.radio("Ver Ranking:", ["General", "De esta Jornada"], horizontal=True)
        u_jug = [u for u in df_u_all['Usuario'].unique() if u not in admins]
        pts_list = []
        for u in u_jug:
            pts = safe_float(df_base[df_base['Usuario'] == u].iloc[0]['Puntos']) if tipo_rank == "General" else 0.0
            u_p = df_p_all[(df_p_all['Usuario'] == u) & (df_p_all['Jornada'] == j_global)] if tipo_rank == "De esta Jornada" else df_p_all[df_p_all['Usuario'] == u]
            ...
            pts_list.append({"Usuario": u, "Puntos": pts})
        ...
        for _, row in df_rank.iterrows():
            ...
            n_perfil, d_perfil, riesgo = obtener_perfil_apostador(df_p_all[df_p_all['Usuario'] == row['Usuario']])
            ...
            if st.button("🚀 Simular"):
                ...
                for p in df_p_all[df_p_all['Usuario'] == usr_s].itertuples():
                    try:
                        ...
                        if p.P_L > p.P_V: sim_d[tl]["Pts"] += 3
                        elif p.P_V > p.P_L: sim_d[tv]["Pts"] += 3
                        else: sim_d[tl]["Pts"] += 1; sim_d[tv]["Pts"] += 1
                    except: continue
                df_sim = pd.DataFrame.from_dict(sim_d, orient='index').reset_index()
                df_sim.columns = ['Equipo','PJ','V','E','D','GF','GC','Pts']
                df_sim = df_sim.sort_values(by=['Pts'], ascending=False).reset_index(drop=True)
                df_sim['Pos'] = range(1, 21)
                st.dataframe(df_sim[['Pos','Equipo','Pts']], hide_index=True, use_container_width=True) #

    with tabs[0]: # APUESTAS (ESTABLE)
        if st.session_state.rol != "admin":
            ...
            for i, (loc, vis) in enumerate(JORNADAS[j_global]):
                ...
                p_env.append({"Usuario": st.session_state.user, "Jornada": j_global, "Partido": m_id, "P_L": pl, "P_V": pv, "Publica": "SI" if pub else "NO"})
            if st.button("💾 Guardar"):
                old = df_p_all[~((df_p_all['Usuario'] == st.session_state.user) & (df_p_all['Jornada'] == j_global))]
                conn.update(worksheet="Predicciones", data=pd.concat([old, pd.DataFrame(p_env)], ignore_index=True)); st.rerun()
