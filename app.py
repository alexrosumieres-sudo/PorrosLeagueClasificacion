            ...
            for j_prog in jornadas_fin:
                ...
                for u in usuarios_jug:
                    ...
                    if not df_base.empty and 'Puntos' in df_base.columns:
                        ...
                        if not m_b.empty:
                            pts_base = safe_float(m_b.iloc[0]['Puntos'])
                    pts_acum = pts_base
            ...
            if historia:
                ...
                for _, row in df_act.sort_values("Posicion").iterrows():
                    ...
                    with c_r2:
                        ...
                        if f_path and pd.notna(f_path) and os.path.exists(str(f_path)):
                            st.image(str(f_path), width=85)
                        else: st.subheader("👤")
                    with c_r3:
    ...
    with tab4: # Detalles
        ...
        if not df_r_j_f.empty:
            ...
            for i, u in enumerate(jugs):
                fp = foto_dict.get(u)
                if fp and pd.notna(fp) and os.path.exists(str(fp)):
                    c_mini[i+1].image(str(fp), width=45)
                else:
            ...
            with adm_tabs[0]:
                ...
                for u in [usr for usr in df_u_all['Usuario'].unique() if usr not in admins]:
                    ...
                    if not df_base.empty and 'Puntos' in df_base.columns:
                        m_b = df_base[df_base['Usuario'] == u]
                        if not m_b.empty:
                            pts_prev = safe_float(m_b.iloc[0]['Puntos'])
                ...
                if os.path.exists(PERFILES_DIR):
                    ...
                    for u in [usr for usr in df_u_all['Usuario'].unique() if usr not in admins]:
                        ...
                        if pd.notna(path_db):
                            ...
                            if nombre_archivo in fotos:
                                idx_f = fotos.index(nombre_archivo) + 1
