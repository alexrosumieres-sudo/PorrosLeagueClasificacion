...
def obtener_perfil_apostador(df_usuario):
    ...
    if pct_locuras > 0.15: return "VISIONARIO ESQUIZO 🔮", f"Cree en los milagros ({locuras} upsets).", riesgo
...
if not st.session_state.autenticado:
    ...
    if c_h2.button("Salir"): st.session_state.autenticado = False; st.rerun()
        ...
        if not df_r_all.empty and not df_p_all.empty:
            ...
            if historia:
                ...
                for _, row in df_act.sort_values("Posicion").iterrows():
                    c_r1, c_r2, c_r3, c_r4 = st.columns([0.5, 1, 4, 1])
                    with c_r1: st.markdown(f"### #{row['Posicion']}")
                    with c_r2:
                        f_path = foto_dict.get(row['Usuario'])
                        if f_path and os.path.exists(f_path): st.image(f_path, width=65)
                        else: st.markdown("👤")
                    with c_r3:
    ...
    with tabs[5]: # Admin
        ...
        else: st.error("Sin permisos de administrador.")
